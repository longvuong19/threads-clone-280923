...
export async function fetchCommunities({
  ...
  try {
    ...
    // Caculate the number of communities to skip based on pageNumber and pageSize
    const skipAmount = (pageNumber - 1) * pageSize;

    // Create a case-intensive regular expression for the provided search string
    const regex = new RegExp(searchString, "i");

    // Create an initial query object to filter communities
    const query: FilterQuery<typeof Community> = {};

    // If searchString is not empty, add the $or operator to match either username or name fields
    ...
    // Define the sort options for the fetch communities based on createdAt field and provided sort order
...
export async function deleteCommunity(communityId: string) {
  try {
    ...
    // Delete all threads assosiated with the community
